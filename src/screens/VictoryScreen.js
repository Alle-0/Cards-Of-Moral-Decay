...
const VictoryScreen = ({ winnerName }) => {
    ...
    const [exiting, setExiting] = React.useState(false); // [NEW] Exit state
    ...
    return (
        <View style={styles.container}>
            ...
            >
                ...
                >
                    Ha dominato la partita con {roomData?.punti?.[winnerName]} punti.
                </Animated.Text>

                {/* Leaderboard Section */}
                <Animated.View entering={FadeIn.delay(1200)} style={styles.leaderboardContainer}>
                    <Text style={[styles.leaderboardTitle, { color: '#888' }]}>CLASSIFICA FINALE</Text>
                    {Object.entries(roomData?.punti || {})
                        .sort(([, scoreA], [, scoreB]) => scoreB - scoreA)
                        .filter(([name]) => name !== winnerName && roomData?.giocatori?.[name]) // Exclude winner and ghost players
                        .map(([name, score], index) => {
                            ...
                            return (
                                <View key={name} style={styles.playerRow}>
                                    ...
                                    <Text style={[styles.playerScore, { color: theme.colors.accent }]}>{score} pt</Text>
                                </View>
                            );
                        })}
                </Animated.View>

                {/* [NEW] Walk of Shame Section */}
                {loserName && (
                    <Animated.View
                        entering={FadeIn.delay(1400).duration(800)}
                        style={styles.shameContainer}
                    >
                        <Text style={styles.shameTitle}>IL FALLIMENTO COMICO DELLA SERATA</Text>
                        <View style={styles.shameRow}>
                            <View style={{ marginRight: 15, position: 'relative' }}>
                                <View style={styles.shameAvatarContainer}>
                                    <LocalAvatar
                                        size={40}
                            ...
                            <View style={styles.shameTextContainer}>
                                ...
                                <Text style={styles.loserPoints}>Ha totalizzato solo {loser?.[1]} punti.</Text>
                            </View>
                        </View>
                    </Animated.View>
                )}
                ...
                {isCreator ? (
                    ...
                    <Animated.View entering={FadeIn.delay(1500)} style={{ width: '80%', marginTop: 10, alignItems: 'center', gap: 20 }}>
                        <Text style={[styles.waitingText, { color: '#e0e0e0', marginTop: 0, fontStyle: 'normal', textAlign: 'center' }]}>
                            In attesa che il creatore inizi una nuova partita...
                        </Text>
...
const styles = StyleSheet.create({
    ...
    shameContainer: {
        width: '100%',
        marginTop: 20,
        backgroundColor: 'rgba(50, 20, 20, 0.4)',
        borderRadius: 16,
        padding: 15,
        borderWidth: 1,
